...
function mostrarNoticias() {
    ...
    noticias.forEach(element => {
        ...
        document.getElementById("DivNoticias").innerHTML += noticiaHTML;
    });
...
function formatearNoticia(noticiajson) {
    link = ""
    if(noticiajson.categoria == "ARTE"){
        link= "noticia.html?noticia=" + noticiajson.id + "";
    }else if(noticiajson.categoria == "DEPORTES"){
        link= "noticia.html?noticia=" + noticiajson.id + "";
    }else if(noticiajson.categoria == "FARÁNDULA"){
        link= "noticia.html?noticia=" + noticiajson.id + "";
    }else if(noticiajson.categoria == "TECNOLOGÍA"){
        link= "noticia.html?noticia=" + noticiajson.id + "";
    }
    formatHTML = `
                <div class='col-sm-12 '>
                    <h3> ${noticiajson.titulo} </h3>
                    <img id='imagenNoticia' src = '${noticiajson.imagen}' />
                    <p> ${noticiajson.fecha} </p>
                    <a href=${link}>Ver Noticia</a>
                </div>`;
    return formatHTML;
}


function mostrarSeleccion(element){
    noticiaHTML = "";
    noticiaHTML = cargarSeleccion(element);
        document.getElementById("puestoNoticia").innerHTML += noticiaHTML;
}


function cargarSeleccion(noticiajson){
    formatHTML = `
                <div class='col-sm-12 '>
                    <h3> ${noticiajson.titulo} </h3>
                    <img id='imagenNoticia' src = '${noticiajson.imagen}' />
                    <p> ${noticiajson.fecha} </p>
                    <p> ${noticiajson.desc_noticia} </p>
                    <a href=${link}>Ver Noticia</a>
                </div>`;
    return formatHTML;
}
console.log("Estoy trabajando este codigo")
cargarNoticias();
mostrarNoticias();
